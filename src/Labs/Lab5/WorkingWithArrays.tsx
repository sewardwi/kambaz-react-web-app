  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <a href={`${API}/${todo.id}/title/${todo.title}`} className="btn btn-primary float-end">
        Update Todo Title with ID = {todo.id} </a>
      ...
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
        Update Todo Completed with ID = {todo.id} </a>
      <FormControl defaultValue={todo.id} className="w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
      <Form.Check className="w-75" id="wd-assignment-completed"
        defaultChecked={todo.completed} onChange={(e) => {
          const target = e.target as HTMLInputElement;
          setTodo({ ...todo, completed: target.checked })}}/>
      <br /><br /><hr />

      <a href={`${API}/${todo.id}/description/${todo.description}`} className="btn btn-primary float-end">
        Update Todo Description with ID = {todo.id} </a>
      <FormControl defaultValue={todo.id} className="w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
